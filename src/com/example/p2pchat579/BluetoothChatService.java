    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if(type == BluetoothChat.MESSAGE_WRITE_TEXT)
                    mHandler.obtainMessage(BluetoothChat.MESSAGE_READ,bytes, -1, buffer).sendToTarget();
                    else if(type != BluetoothChat.MESSAGE_WRITE_TEXT)
                    {
                    	
                    	if(type == BluetoothChat.MESSAGE_WRITE_PICTURE )
                        {
                    	 ...
                    	 {
                    		 mHandler.obtainMessage(BluetoothChat.MESSAGE_READ,length, -1, imageBuffer).sendToTarget();                   	
                    		  //reset Buffer
                    		 imageBuffer = null;
        ...
        public void write(byte[] buffer, int type) {
            try {
                ...
                mHandler.obtainMessage(BluetoothChat.MESSAGE_WRITE, -1, type, resBuff)
                        .sendToTarget();
            } catch (IOException e) {
